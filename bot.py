...
Язык: строго русский.
ЗАДАЧА:
1. ТЕКУЩАЯ ФОРМА L8 (КРАТКИЙ БАЗОВЫЙ ФИЛЬТР):
   - Разбор последних 8 официальных матчей (W/L) каждого игрока: счета, покрытие, отказы, спад.
2. МЕТРИКИ TENNIS ABSTRACT:
   - Surface Elo, Hold %, Break %, Dominance Ratio (DR), 2nd Serve Win %.
3. ВЫБОР ПО ЛИНИИ (СТРОГО БЕЗ ПЕРЕБОРА ВСЕХ ВАРИАНТОВ):
   - Выбери ровно 1 ГЛАВНЫЙ выбор с максимальным перевесом (Value Pick).
   - Выбери ровно 1 АЛЬТЕРНАТИВНЫЙ выбор / страховку (Risk Nullifier), которая точечно перекрывает главный риск фаворита (например, фора по геймам/сетам или тотал).
ФОРМАТ ВЫДАЧИ:
📊 **Форма L8 и метрики**
• [Игрок 1]: L8: [W/L] (суть за 8 матчей, отказы) | Elo [X] | Hold [X]% | Break [X]% | DR [X]
• [Игрок 2]: L8: [W/L] (суть за 8 матчей, отказы) | Elo [X] | Hold [X]% | Break [X]% | DR X
📋 **Точечный вердикт по линии**

| Маркет | Выбор | Обоснование перевеса (Edge) | Главная опасность |
| :--- | :--- | :--- | :--- |
| **Основной (Best Value)** | ... | ... | ... |
| **Страховка (Risk Nullifier)** | ... | ... | ... |
...
def handle_msg(message):
    ...
    msg = bot.reply_to(message, "⏳ Анализирую матч и подбираю страховку...")
